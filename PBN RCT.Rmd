...
followup_data <- read.csv("followups_5.9.24.csv") 
...
followup_data <- followup_data %>%
  arrange(Participant_ID, desc(Progress), RecordedDate)
followup_data <- followup_data %>%
...
#Select variables that will be used for analysis / in imputation
###SINCE REMOVING TIMING AND TEXT-BASED VARIABLES, WILL NEED TO LOOK AT SEPERATELY ELSEWHERE###
###WHAT ELSE SHOULD WE INCLUDE####
included_participants <- included_participants %>%
   select(Participant_ID, treatment_group, starts_with(c("b_dem", "b_current", "b_past", "b_unmet", "b_hunger", "b_bhs", "b_shs_agency", "b_biss", "b_fas", "b_biaaq", "b_chime", "b_mfq", "b_edeq", "b_mddi", "pi_pfs", "pi_bhs", "pi_shs_agency", "pi_biss", "pi_fas", "pi_biaaq", "pi_chime", "f_new", "f_bhs", "f_shs_agency", "f_biss", "f_fas", "f_biaaq", "f_chime", "f_mfq", "f_edeq", "f_mddi"))) %>%
  select(-contains("free"), -contains("TEXT"), -contains("_tim_"), -contains("pfs_like"), -contains("pfs_change"), -contains("pfs_other")) %>%
  mutate_all(as.numeric)
###NEED TO CHECK### - mutate_all(as.numeric) produces two warning messages that introduce NAs in b_dem_race_ethnicity and b_dem_gender but I'm not sure why. I am pretty sure missForest() can only work with numeric and categorical variables (meaning I think we have to override variables like treatment_group that were originally logical). Will this cause problems later?
```
...
```{r handling missing data without imputation}
...
check_pfs_partial <- included_participants %>%
  filter(!is.na(pi_pfs_1) | !is.na(pi_pfs_2) | !is.na(pi_pfs_3) | !is.na(pi_pfs_4) | !is.na(pi_pfs_5) | !is.na(pi_pfs_6) | !is.na(pi_pfs_7))
#Compare to a dataframe of folks who completed all of the PFS items
check_pfs_all <- included_participants %>%
...
missing_participant_ids <- setdiff(check_pfs_partial$Participant_ID, check_pfs_all$Participant_ID)
print(missing_participant_ids)
#The participant with missing data is 64531 
###FIGURE OUT WHAT TO DO WITH THEM###

#Create score and filter for program feedback completers (just renaming for later code here since the data frame was made above)
included_participants_pfs <- check_pfs_all
...
###NEED TO CHECK### - I think included_participants can now be considered analysis-ready for imputation (since it dropped participants with more than 5% missing baseline data and includes all the variables that I think will be useful in imputation). There are a few extra variables that I've used along the way such as Participant_ID, treatment_group, percent_missing, and more_than_5_percent missing. Keep or drop these for imputation?
```
...
#imputed_data <- missForest(included_participants)
...
###NOTE TO SELF### - Since we will be imputing data, filtering for cases with complete proximal outcomes might eventually not be necessary. Just keeping for now since the code chunks below cannot work with missing values and therefore need these dataframes.
included_participants_complete_proximal <- included_participants %>%
...
included_participants_complete_proximal <- included_participants %>%
  semi_join(included_participants_complete_proximal, by = "Participant_ID")

#Create a dataframe with complete PBN participants only
pbn_complete <- included_participants_complete_proximal %>%
  filter(treatment_group == TRUE)

#Create a dataframe with complete Control participants only
control_complete <- included_participants_complete_proximal %>%
  filter(treatment_group == FALSE)
```

```{r descriptives for proximal outcomes}
descriptives <- included_participants_complete_proximal %>%
...
#Hopelessness
lm_bhs <- lm(pi_bhs_sum ~ b_bhs_sum + treatment_group, data = included_participants_complete_proximal)
summary(lm_bhs)
...
#Agency
lm_shs_agency <- lm(pi_shs_agency_sum ~ b_shs_agency_sum + treatment_group, data = included_participants_complete_proximal)
...
#Body image state
lm_biss <- lm(pi_biss_sum ~ b_biss_sum + treatment_group, data = included_participants_complete_proximal)
summary(lm_biss)
...
lm_fas <- lm(pi_fas_sum ~ b_fas_sum + treatment_group, data = included_participants_complete_proximal)
summary(lm_fas)
...
lm_biaaq <- lm(pi_biaaq_sum ~ b_biaaq_sum + treatment_group, data = included_participants_complete_proximal)
summary(lm_biaaq)
...
lm_chime <- lm(pi_chime_sum ~ b_chime_sum + treatment_group, data = included_participants_complete_proximal)
summary(lm_chime)
...
```{r qual data}
###COME BACK TO THIS### - Does not run right now because I removed text variables from  included_participants for imputation
...
#Age
included_participants_complete_proximal %>%
  count(b_dem_age) %>%
