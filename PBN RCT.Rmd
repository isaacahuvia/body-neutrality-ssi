...
participants_with_followp <- included_participants %>%
  left_join(followup_data_deduplicated, by = "Participant_ID")

#Create a binary variable to identify the treatment group
participants_with_followp <- participants_with_followp %>%
...
participants_with_followp_pfs_missing <- participants_with_followp %>%
...
participants_with_followp_pfs <- participants_with_followp %>%
...
missing_participant_ids <- setdiff(participants_with_followp_pfs_missing$Participant_ID, participants_with_followp_pfs$Participant_ID)
...
participants_with_followp <- participants_with_followp %>%
  mutate(percent_missing = rowMeans(
    is.na(select(participants_with_followp, b_mfq_1, b_mfq_2, b_mfq_3, b_mfq_4, b_mfq_5, b_mfq_6, b_mfq_7, b_mfq_8, b_mfq_9, b_mfq_10, b_mfq_11, b_mfq_12, b_mfq_13, b_edeq_1, b_edeq_2, b_edeq_3, b_edeq_4, b_edeq_5, b_edeq_6, b_edeq_7, b_edeq_8, b_edeq_9, b_edeq_10)), 
    na.rm = TRUE)) %>%
...
#In sum, there were originally 220 participants_with_followp and now there are 218 (only had to drop two participants prior to imputation)

#Drop remaining columns that are not useful for imputation
participants_with_followp_for_imputation <- participants_with_followp %>%
...
#imputed_data <- missForest(participants_with_followp_for_imputation)
...
participants_with_followp_complete_proximal <- participants_with_followp %>%
...
participants_with_followp_complete_proximal <- participants_with_followp %>%
  semi_join(participants_with_followp_complete_proximal, by = "Participant_ID")

#Create a dataframe with complete PBN participants only
pbn_complete <- participants_with_followp_complete_proximal %>%
  filter(treatment_group == TRUE)

#Create a dataframe with complete Control participants only
control_complete <- participants_with_followp_complete_proximal %>%
  filter(treatment_group == FALSE)
```

```{r descriptives for proximal outcomes}
descriptives <- participants_with_followp_complete_proximal %>%
...
#Hopelessness
lm_bhs <- lm(pi_bhs_sum ~ b_bhs_sum + treatment_group, data = participants_with_followp_complete_proximal)
summary(lm_bhs)
...
#Agency
lm_shs_agency <- lm(pi_shs_agency_sum ~ b_shs_agency_sum + treatment_group, data = participants_with_followp_complete_proximal)
...
#Body image state
lm_biss <- lm(pi_biss_sum ~ b_biss_sum + treatment_group, data = participants_with_followp_complete_proximal)
summary(lm_biss)
...
lm_fas <- lm(pi_fas_sum ~ b_fas_sum + treatment_group, data = participants_with_followp_complete_proximal)
summary(lm_fas)
...
lm_biaaq <- lm(pi_biaaq_sum ~ b_biaaq_sum + treatment_group, data = participants_with_followp_complete_proximal)
summary(lm_biaaq)
...
lm_chime <- lm(pi_chime_sum ~ b_chime_sum + treatment_group, data = participants_with_followp_complete_proximal)
summary(lm_chime)
...
```{r qual data}
###COME BACK TO THIS### - Does not run right now because I removed text variables from  participants_with_followp for imputation

#Help seeking and unmet treatment for the full included sample, regardless of completion or randomization
qual_help_seeking <- participants_with_followp %>% 
...
pfs_qual_intervention <- participants_with_followp_pfs %>%
  filter(treatment_group == TRUE) %>%
...
#Age
participants_with_followp_complete_proximal %>%
  count(b_dem_age) %>%
