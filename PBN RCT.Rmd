...
#The reason the numbers are weird below! We don't actually have a completer_baseline N of 250 because we decided after the fact to exclude 30 people that we already compensated.
...
incomplete_intervention <- merged_data %>%
  ...
  semi_join(randomized,  by = "Participant_ID")
n_incomplete_intervention <- nrow(incomplete_intervention)
#N=15
...
n_randomized_intervention_complete <- nrow(randomized_intervention_complete)
#N=100
...
randomized_control_complete <- randomized_control %>%
  select(Participant_ID) %>%
  semi_join(completer_intervention %>% select(Participant_ID), by = "Participant_ID")
n_randomized_control_complete <- nrow(randomized_control_complete)
#N=105
...
n_randomized_intervention_incomplete <- nrow(randomized_intervention_incomplete)
#N=5

#Control group participants who passed data integrity checks and did not their intervention
randomized_control_incomplete <- randomized_control %>%
  select(Participant_ID) %>%
  semi_join(incomplete_intervention %>% select(Participant_ID), by = "Participant_ID")
n_randomized_control_incomplete <- nrow(randomized_control_incomplete)
#N=10

#Participants who received follow-up emails (have to hard code based on the listservs we created manually)
invited_followup <- participant_database %>%
...
n_invited_followup <- nrow(invited_followup)
#N=220

#Participants who completed the follow-up
completed_followup <- participant_database %>%
  filter(Follow.Up.Completion == "Yes")
n_completed_followup <- nrow(completed_followup)
#N=120

#Participants who did not complete the follow-up
incomplete_followup <- participant_database %>%
  filter(Follow.Up.Completion != "Yes" & Include.in.Analysis == "Yes")
n_incomplete_followup <- nrow(incomplete_followup)
#N=100
...
completed_followup <- completed_followup %>%
  mutate(Participant_ID = as.character(Participant_ID))
incomplete_intervention_completed_followup <- incomplete_intervention %>%
  select(Participant_ID) %>%
  semi_join(completed_followup %>% select(Participant_ID), by = "Participant_ID")
n_incomplete_intervention_completed_followup <- nrow(incomplete_intervention_completed_followup)
#N=1

#Participants who were randomized but did not complete their intervention who did not complete their follow-up
incomplete_followup <- incomplete_followup %>%
  mutate(Participant_ID = as.character(Participant_ID))
...
n_incomplete_intervention_incomplete_followup <- nrow(incomplete_intervention_incomplete_followup)
#N=14
```

```{r}

#TO DO: At some point, will need to use the below code to get rid of duplicates in the actual follow-up data (above is using the participant database)
