...
all_data <- all_data %>%
  mutate(
    ...
    intervention_completed = case_when(
...
incomplete_baseline <- all_data %>% 
  filter(is.na(`Measures Completed.baseline`) & Include.in.Analysis == "Yes")
...
completer_intervention <- all_data %>% 
  filter(Include.in.Analysis == "Yes", intervention_completed == "Completed" | control_completed == "Completed")  %>%
...
incomplete_intervention <- all_data %>%
  filter((`Condition` == "Intervention" & (is.na(`Intervention Completed`) | `Intervention Completed` != "Intervention Completed")) | (`Condition` == "Control" & (is.na(`Control Completed`) | `Control Completed` != "Control Completed"))) %>%
  semi_join(randomized,  by = "Participant_ID")
...
#N=1
####HOW WOULD THIS HAVE CHANGED FROM 1 TO 0 WITH THE CHANGES I MADE ON THIS BRANCH?####
...
randomized_selected <- randomized %>%
...
# More variables: completed baseline, completed followup
...
qual_data_intervention <- randomized_intervention_pfs_complete %>%
 select(Participant_ID, pi_pfs_like, pi_pfs_change, pi_pfs_other)

#Dataframe for qualitative analysis (control completers with PFS data)
qual_data_control <- randomized_control_pfs_complete %>%
 select(Participant_ID, pi_pfs_like, pi_pfs_change, pi_pfs_other)
...
#imputed_randomized_quant <- round(imputed_randomized_quant) #Since the algorithm introduces decimals which is not appropriate for this data,  round to the nearest whole number. UPDATE FROM ISAAC: It may actually make sense to include decimals - discuss this. It currently doesn't run with rounding
...
#imputed_completer_intervention_quant <- round(imputed_completer_intervention_quant) #Since the algorithm introduces decimals which is not appropriate for this data,  round to the nearest whole number. UPDATE FROM ISAAC: It may actually make sense to include decimals - discuss this. It may actually make sense to include decimals - discuss this. It currently doesn't run with rounding
...
#imputed_completer_baseline_quant <- round(imputed_completer_baseline_quant) #Since the algorithm introduces decimals which is not appropriate for this data,  round to the nearest whole number. UPDATE FROM ISAAC: It may actually make sense to include decimals - discuss this. It may actually make sense to include decimals - discuss this. It currently doesn't run with rounding
...
lm_edeq_itt <- lm(f_edeq_global ~ b_edeq_global + treatment_group,  data = imputed_randomized_quant)
summary(lm_edeq_itt)
...
```{r demographics - ITT non-imputed}
...
randomized_quant %>% 
  summarize(mean = mean(b_mfq_sum, na.rm = TRUE), 
            sd = sd(b_mfq_sum, na.rm = TRUE))

#Eating disorder symptoms at baseline
randomized_quant %>% 
  mutate(b_edeq_dietary_restraint = b_edeq_1 + b_edeq_2 + b_edeq_3,
         b_edeq_body_dissatisfaction = b_edeq_4 + b_edeq_5 + b_edeq_6,
         b_edeq_overvaluation = b_edeq_7 + b_edeq_8 + b_edeq_9 + b_edeq_10) %>% 
  select(b_edeq_dietary_restraint, b_edeq_body_dissatisfaction, b_edeq_overvaluation) %>% 
   summarize(across(everything(),  
                    list(mean = mean,  sd = sd))) 

#I think I messed up these analyses and need to use global EDE-Q scores (which is an average of the three subscales) throughout rather than the sum of all items
```