...
#N and percent of intervention participants with sum score >4 (Strongly Agree only),  reflect overall perceived SSI acceptability
df_completer_pfs_nonimputed_intervention %>%
  mutate(pi_pfs_sum_scores = pi_pfs_sum / 7,  
...
#N and percent of control participants with sum score >4 (selects Strongly Agree only),  reflect overall perceived SSI acceptability
df_completer_pfs_nonimputed_control %>%
  mutate(pi_pfs_sum_scores = pi_pfs_sum / 7,  
         pi_pfs_sum_scores_acceptable = pi_pfs_sum_scores > 4) %>%
  summarise(pi_pfs_sum_scores_acceptable_percent = scales::percent(mean(pi_pfs_sum_scores_acceptable)))
...
df_bhs <- descriptives %>%
  select(treatment_group, b_bhs_sum_mean, pi_bhs_sum_mean) %>%
...
df_shs_agency <- descriptives %>%
  select(treatment_group, b_shs_agency_sum_mean, pi_shs_agency_sum_mean) %>%
...
df_fas <- descriptives %>%
  select(treatment_group, b_fas_sum_mean, pi_fas_sum_mean) %>%
...
df_biss <- descriptives %>%
  select(treatment_group, b_biss_sum_mean, pi_biss_sum_mean) %>%
...
grid.arrange(plot_bhs, plot_shs_agency, plot_fas, plot_biss, ncol = 2)
```
