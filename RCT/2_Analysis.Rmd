...
df_bhs <- descriptives_itt %>%
  dplyr::select(treatment_group, b_bhs_sum_mean, pi_bhs_sum_mean) %>%
  rename(Pre = b_bhs_sum_mean, Post = pi_bhs_sum_mean) %>%
  pivot_longer(cols = c(Pre, Post), names_to = "time", values_to = "score") %>%
  mutate(time = factor(time, levels = c("Pre", "Post")))  # Set order of levels
plot_bhs <- ggplot(df_bhs, aes(x = time, y = score, group = treatment_group, color = treatment_group)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(title = "Hopelessness", x = "Time", y = "Score", color = "Treatment Group") +
  theme_classic()
plot_bhs

df_shs_agency <- descriptives_itt %>%
  dplyr::select(treatment_group, b_shs_agency_sum_mean, pi_shs_agency_sum_mean) %>%
  rename(Pre = b_shs_agency_sum_mean, Post = pi_shs_agency_sum_mean) %>%
  pivot_longer(cols = c(Pre, Post), names_to = "time", values_to = "score") %>%
  mutate(time = factor(time, levels = c("Pre", "Post")))  # Set order of levels
plot_shs_agency <- ggplot(df_shs_agency, aes(x = time, y = score, group = treatment_group, color = treatment_group)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(title = "Agency", x = "Time", y = "Score", color = "Treatment Group") +
  theme_classic()
plot_shs_agency

df_fas <- descriptives_itt %>%
  dplyr::select(treatment_group, b_fas_sum_mean, pi_fas_sum_mean) %>%
  rename(Pre = b_fas_sum_mean, Post = pi_fas_sum_mean) %>%
  pivot_longer(cols = c(Pre, Post), names_to = "time", values_to = "score") %>%
  mutate(time = factor(time, levels = c("Pre", "Post")))  # Set order of levels
plot_fas <- ggplot(df_fas, aes(x = time, y = score, group = treatment_group, color = treatment_group)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(title = "Functionality Appreciation", x = "Time", y = "Score", color = "Treatment Group") +
  theme_classic()
plot_fas

df_biss <- descriptives_itt %>%
  dplyr::select(treatment_group, b_biss_sum_mean, pi_biss_sum_mean) %>%
  rename(Pre = b_biss_sum_mean, Post = pi_biss_sum_mean) %>%
  pivot_longer(cols = c(Pre, Post), names_to = "time", values_to = "score") %>%
  mutate(time = factor(time, levels = c("Pre", "Post")))  # Set order of levels
plot_biss <- ggplot(df_biss, aes(x = time, y = score, group = treatment_group, color = treatment_group)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(title = "State Body Satisfaction", x = "Time", y = "Score", color = "Treatment Group") +
  theme_classic()
plot_biss
```
...
interaction_plot <- df_randomized_imputed %>%
 ...
 scale_linetype_manual(
  values = c("Below Threshold" = "dashed", "Above Threshold" = "solid")
  ) +
  theme_classic()
interaction_plot
...
long_proximal_outcomes <- df_randomized_imputed %>%
  ...
  mutate(
    ...
    measure = case_when(
      ...
      measure == "biss_sum" ~ "State Body Satisfaction"
    ),
...
proximal_plot <- ggplot(long_proximal_outcomes, aes(x = time, y = score, group = treatment_group, color = treatment_group)) +
  ...
  facetted_pos_scales(
    y = list(
      ...
      measure == "State Body Satisfaction" ~ scale_y_continuous(
        limits = c(6, 54),
...
```{r additional analyses per revise and resubmit}
#Extract cis and trans demographics columns
gender_cols <- imputation_ready_with_IDs %>% 
  dplyr::select(starts_with("b_dem_gender_")) %>% 
  names()
cis_cols   <- c("b_dem_gender_1", "b_dem_gender_2")
trans_cols <- setdiff(gender_cols, cis_cols)

#Make a new column setting if someone is cis or trans
imputation_ready_with_IDs <- imputation_ready_with_IDs %>%
  mutate(
...
gender_group_info <- imputation_ready_with_IDs %>%
  dplyr::select(Participant_ID, gender_group)

#Bring new gender variable into imputed data for the full randomized sample (used in ITT outcome analyses)
df_randomized_imputed <- df_randomized_imputed %>%
  left_join(gender_group_info, by = "Participant_ID")

#And into the dataset with intervention completers (used in TOT outcome analyses)
df_completer_intervention_imputed <- df_completer_intervention_imputed %>%
  left_join(gender_group_info, by = "Participant_ID")

#And into the dataset with PFS completers for intervention participants (used for acceptability analyses)
df_completer_pfs_nonimputed_intervention <- df_completer_pfs_nonimputed_intervention %>%
  left_join(gender_group_info, by = "Participant_ID")

#Count trans and cis participants in each group within the full randomized sample
df_randomized_imputed %>%
  count(treatment_group, gender_group)

#Look at interaction between gender group and treatment group (controlling for baseline scores) on primary outcomes - ITT
lm_explore2_edeq_itt <- lm(f_edeq_global ~ b_edeq_global + gender_group + treatment_group + gender_group*treatment_group, data = df_randomized_imputed)
summary(lm_explore2_edeq_itt)
lm_explore2_mfq_itt <- lm(f_mfq_sum ~ b_mfq_sum + gender_group + treatment_group + gender_group*treatment_group, data = df_randomized_imputed)
summary(lm_explore2_mfq_itt)
summary(lm_explore2_mfq_itt)$coefficients["gender_grouptrans:treatment_groupIntervention", "Pr(>|t|)"] #original output not showing 3 decimals for some reason
#No interaction effects

#Check for baseline differences between gender groups on primary outcomes - ITT
t.test(b_edeq_global ~ gender_group, data = df_randomized_imputed)
t.test(b_mfq_sum ~ gender_group, data = df_randomized_imputed)
#No significant differences

#Look at interaction between gender group and treatment group (controlling for baseline scores) on primary outcomes - TOT
lm_explore2_edeq_tot <- lm(f_edeq_global ~ b_edeq_global + gender_group + treatment_group + gender_group*treatment_group, data = df_completer_intervention_imputed)
summary(lm_explore2_edeq_tot)
lm_explore2_mfq_tot <- lm(f_mfq_sum ~ b_mfq_sum + gender_group + treatment_group + gender_group*treatment_group, data = df_completer_intervention_imputed)
summary(lm_explore2_mfq_tot)
#No interaction effects

#Check for baseline differences between gender groups on primary outcomes - TOT
t.test(b_edeq_global ~ gender_group, data = df_completer_intervention_imputed)
t.test(b_mfq_sum ~ gender_group, data = df_completer_intervention_imputed)
#Baseline depression scores were significantly higher for trans participants

#Effect sizes for EDEQ reductions in trans and cis participants, by condition - ITT

#Descriptives for trans in intervention
edeq_stats_intervention_itt_trans <- df_randomized_imputed %>%
  filter(gender_group == "trans", treatment_group == "Intervention") %>% 
  dplyr::select(b_edeq_global, f_edeq_global) %>%
  mutate(edeq_global_change_pre_followup = f_edeq_global - b_edeq_global) %>%
  summarize(n = n(), 
            across(everything(),  list(mean = mean,  sd = sd))) 

#Descriptives for trans in control
edeq_stats_control_itt_trans <- df_randomized_imputed %>%
  filter(gender_group == "trans", treatment_group == "Control") %>% 
  dplyr::select(b_edeq_global, f_edeq_global) %>%
  mutate(edeq_global_change_pre_followup = f_edeq_global - b_edeq_global) %>%
  summarize(n = n(), 
            across(everything(),  list(mean = mean,  sd = sd))) 

#within-person d(av) for trans in intervention
dav_intervention_edeq_itt_trans <- d.dep.t.avg(m1 = edeq_stats_intervention_itt_trans$b_edeq_global_mean, 
                                         m2 = edeq_stats_intervention_itt_trans$f_edeq_global_mean, 
                                         sd1 = edeq_stats_intervention_itt_trans$b_edeq_global_sd, 
                                         sd2 = edeq_stats_intervention_itt_trans$f_edeq_global_sd, 
                                         n = edeq_stats_intervention_itt_trans$n, 
                                         a = .05) %>%
  pluck("estimate")

#within-person d(av) for trans in control
dav_control_edeq_itt_trans <- d.dep.t.avg(m1 = edeq_stats_control_itt_trans$b_edeq_global_mean, 
                                         m2 = edeq_stats_control_itt_trans$f_edeq_global_mean, 
                                         sd1 = edeq_stats_control_itt_trans$b_edeq_global_sd, 
                                         sd2 = edeq_stats_control_itt_trans$f_edeq_global_sd, 
                                         n = edeq_stats_control_itt_trans$n, 
                                         a = .05) %>%
  pluck("estimate")

#Descriptives for cis in intervention
edeq_stats_intervention_itt_cis <- df_randomized_imputed %>%
  filter(gender_group == "cis", treatment_group == "Intervention") %>% 
  dplyr::select(b_edeq_global, f_edeq_global) %>%
  mutate(edeq_global_change_pre_followup = f_edeq_global - b_edeq_global) %>%
  summarize(n = n(), 
            across(everything(),  list(mean = mean,  sd = sd))) 

#Descriptives for cis in control
edeq_stats_control_itt_cis <- df_randomized_imputed %>%
  filter(gender_group == "cis", treatment_group == "Control") %>% 
  dplyr::select(b_edeq_global, f_edeq_global) %>%
  mutate(edeq_global_change_pre_followup = f_edeq_global - b_edeq_global) %>%
  summarize(n = n(), 
            across(everything(),  list(mean = mean,  sd = sd))) 

#within-person d(av) for cis in intervention
dav_intervention_edeq_itt_cis <- d.dep.t.avg(m1 = edeq_stats_intervention_itt_cis$b_edeq_global_mean, 
                                         m2 = edeq_stats_intervention_itt_cis$f_edeq_global_mean, 
                                         sd1 = edeq_stats_intervention_itt_cis$b_edeq_global_sd, 
                                         sd2 = edeq_stats_intervention_itt_cis$f_edeq_global_sd, 
                                         n = edeq_stats_intervention_itt_cis$n, 
                                         a = .05) %>%
  pluck("estimate")

#within-person d(av) for cis in control
dav_control_edeq_itt_cis <- d.dep.t.avg(m1 = edeq_stats_control_itt_cis$b_edeq_global_mean, 
                                         m2 = edeq_stats_control_itt_cis$f_edeq_global_mean, 
                                         sd1 = edeq_stats_control_itt_cis$b_edeq_global_sd, 
                                         sd2 = edeq_stats_control_itt_cis$f_edeq_global_sd, 
                                         n = edeq_stats_control_itt_cis$n, 
                                         a = .05) %>%
  pluck("estimate")

#Effect sizes for MFQ reductions in trans and cis participants, by condition - ITT

#Descriptives for trans in intervention
mfq_stats_intervention_itt_trans <- df_randomized_imputed %>%
  filter(gender_group == "trans", treatment_group == "Intervention") %>% 
  dplyr::select(b_mfq_sum, f_mfq_sum) %>%
  mutate(mfq_sum_change_pre_followup = f_mfq_sum - b_mfq_sum) %>%
  summarize(n = n(), 
...
#Descriptives for trans in control
mfq_stats_control_itt_trans <- df_randomized_imputed %>%
  filter(gender_group == "trans", treatment_group == "Control") %>% 
  dplyr::select(b_mfq_sum, f_mfq_sum) %>%
  mutate(mfq_sum_change_pre_followup = f_mfq_sum - b_mfq_sum) %>%
  summarize(n = n(), 
...
#within-person d(av) for trans in intervention
dav_intervention_mfq_itt_trans <- d.dep.t.avg(m1 = mfq_stats_intervention_itt_trans$b_mfq_sum_mean, 
                                         m2 = mfq_stats_intervention_itt_trans$f_mfq_sum_mean, 
                                         sd1 = mfq_stats_intervention_itt_trans$b_mfq_sum_sd, 
                                         sd2 = mfq_stats_intervention_itt_trans$f_mfq_sum_sd, 
                                         n = mfq_stats_intervention_itt_trans$n, 
                                         a = .05) %>%
  pluck("estimate")

#within-person d(av) for trans in control
dav_control_mfq_itt_trans <- d.dep.t.avg(m1 = mfq_stats_control_itt_trans$b_mfq_sum_mean, 
                                         m2 = mfq_stats_control_itt_trans$f_mfq_sum_mean, 
                                         sd1 = mfq_stats_control_itt_trans$b_mfq_sum_sd, 
                                         sd2 = mfq_stats_control_itt_trans$f_mfq_sum_sd, 
                                         n = mfq_stats_control_itt_trans$n, 
                                         a = .05) %>%
  pluck("estimate")

#Descriptives for cis in intervention
mfq_stats_intervention_itt_cis <- df_randomized_imputed %>%
  filter(gender_group == "cis", treatment_group == "Intervention") %>% 
  dplyr::select(b_mfq_sum, f_mfq_sum) %>%
  mutate(mfq_sum_change_pre_followup = f_mfq_sum - b_mfq_sum) %>%
  summarize(n = n(), 
...
#Descriptives for cis in control
mfq_stats_control_itt_cis <- df_randomized_imputed %>%
  filter(gender_group == "cis", treatment_group == "Control") %>% 
  dplyr::select(b_mfq_sum, f_mfq_sum) %>%
  mutate(mfq_sum_change_pre_followup = f_mfq_sum - b_mfq_sum) %>%
  summarize(n = n(), 
...
#within-person d(av) for cis in intervention
dav_intervention_mfq_itt_cis <- d.dep.t.avg(m1 = mfq_stats_intervention_itt_cis$b_mfq_sum_mean, 
                                         m2 = mfq_stats_intervention_itt_cis$f_mfq_sum_mean, 
                                         sd1 = mfq_stats_intervention_itt_cis$b_mfq_sum_sd, 
                                         sd2 = mfq_stats_intervention_itt_cis$f_mfq_sum_sd, 
                                         n = mfq_stats_intervention_itt_cis$n, 
                                         a = .05) %>%
  pluck("estimate")

#within-person d(av) for cis in control
dav_control_mfq_itt_cis <- d.dep.t.avg(m1 = mfq_stats_control_itt_cis$b_mfq_sum_mean, 
                                         m2 = mfq_stats_control_itt_cis$f_mfq_sum_mean, 
                                         sd1 = mfq_stats_control_itt_cis$b_mfq_sum_sd, 
                                         sd2 = mfq_stats_control_itt_cis$f_mfq_sum_sd, 
                                         n = mfq_stats_control_itt_cis$n, 
                                         a = .05) %>%
  pluck("estimate")

#Differences in PBN acceptability trans vs cis
trans_intervention_pfs <- df_completer_pfs_nonimputed_intervention %>%
  filter(gender_group == "trans")
cis_intervention_pfs <- df_completer_pfs_nonimputed_intervention %>%
  filter(gender_group == "cis")
t.test(cis_intervention_pfs$pi_pfs_sum, trans_intervention_pfs$pi_pfs_sum)
t.test(cis_intervention_pfs$pi_pfs_sum, trans_intervention_pfs$pi_pfs_sum)$p.value
#No significant difference

#Differences in PBN acceptability predicted by on age
age_acceptability <- lm(pi_pfs_sum ~ b_dem_age, df_completer_pfs_nonimputed_intervention)
summary(age_acceptability)
#The p value is 0.6, lets see in which direction
summary_by_age <- df_completer_pfs_nonimputed_intervention %>%
  dplyr::select(b_dem_age, pi_pfs_sum) %>%
  group_by(b_dem_age) %>%
  summarize(
    n = n(),
    mean_total = mean(pi_pfs_sum, na.rm = TRUE),
    sd_total = sd(pi_pfs_sum, na.rm = TRUE),
    mean_item = mean_total / 7,
    sd_item = sd_total / 7
  )
summary_by_age
ggplot(summary_by_age, aes(x = b_dem_age, y = mean_item)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = mean_item - sd_item, ymax = mean_item + sd_item), width = 0.2) +
  labs(x = "Age", y = "Mean PFS Score", title = "PFS Scores by Age")
#Older kids actually rating PBN as better (nonsignificantly) and younger kids (smaller ns) have larger SDs

#Effect size of between-group differences in SSI acceptability
cohen.d(df_completer_pfs_nonimputed_intervention$pi_pfs_sum,
  df_completer_pfs_nonimputed_control$pi_pfs_sum,
  paired = FALSE)
```